...
async def test_agent_on_message_send(agent_app: AgentEngineApp) -> None:
    ...
    message_data = {
        "message": {
            ...
            "content": [{"text": "Please list my lorebooks."}],
            "role": "ROLE_USER",
        },
    }
    ...
    # Poll briefly for completion; if still in progress, verify task state is valid.
    try:
        final_response = await poll_task_completion(
            agent_app,
            response["task"]["id"],
            max_attempts=25,
            interval=0.2,
        ...
        latest = await agent_app.on_get_task(
            request=build_get_request({"id": response["task"]["id"]}),
            context=None,
        )
        state = latest.get("status", {}).get("state", "")
        assert state in {
            "TASK_STATE_SUBMITTED",
            "TASK_STATE_WORKING",
            "TASK_STATE_INPUT_REQUIRED",
        }, f"Unexpected task state: {state}"
        return

    # Verify artifacts when task completes quickly.
